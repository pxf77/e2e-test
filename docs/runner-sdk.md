...
```json
{
  ...
  "expected": {
    "status": 200,
    "json": {"status": "ok", "service.name": "demo"}
  }
}
```
...
Mobile Runner 不绑定特定 Appium client。通过 `mobile_fixtures.command` 调用现有原生测试项目：
...
```json
{
  "mobile_fixtures": {
    ...
    "cwd": "mobile-tests",
    "artifact_paths": ["mobile-tests/results"]
  }
}
```

没有 command 时返回显式 `blocked`，不会伪造通过结果。
...
5. 补 `execution-result@v2` 契约测试和失败分类测试。
